...
temp_bucket_name = os.environ.get('GCP_TEMP_BUCKET_NAME')
pyspark_file_uri = f'gs://{bucket_name}/spark_join_bigquery.py'
...
def submit_pyspark_job():
    ...
    job = {
        ...
        'pyspark_job': {
            ...
            'args': [bucket_name, temp_bucket_name, project_id]
        }
    }

    # Submit the job
    result = client.submit_job(project_id=project_id, 
                                region=region,
                                job=job)
